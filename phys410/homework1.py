...
def problem_four(part):
    ...
    if part.lower() == "a":
        ...
        phi0 = np.deg2rad(90)
        omega0 = 0
    else: raise ValueError("Part must be 'a' or 'b'.")
    ...
    plt.plot(tspan, phi, label="Numerical solution", color="navy")
    plt.plot(tspan, phi_small, ls ="--", label="Small-angle approximation", color="red")
    plt.xlabel("Time [s]")
